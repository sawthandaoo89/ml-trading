...
def assess_portfolio(  		  	   		 	 	 		  		  		    	 		 		   		 		  
    ...
    allocs = np.array(allocs, dtype=float)
    ...
    # rfr is per sample period (same frequency as daily_returns)
    sr = np.sqrt(sf) * ((daily_returns - rfr).mean()) / sddr if sddr != 0 else 0.0
